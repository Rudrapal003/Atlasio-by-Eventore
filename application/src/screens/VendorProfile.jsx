...
const SAMPLE_REVIEWS = [
  ...
  { id: 2, author: 'Daniel R.',    rating: 5, body: 'Booked through Eventore for our engagement — super smooth, no surprise charges, vendor was responsive.', verified: true, date: '1 month ago' },
...
const VendorProfile = () => {
  ...
  React.useEffect(() => {
    const fetchPackages = async () => {
      ...
      if (localStorage.getItem('QA_MODE') === 'true') {
        setPackages([
          { id: 1, name: 'Premium Full Service', price: 5000, type: 'flat', description: 'Everything included.', features: ['8 hours coverage', 'Full setup'] },
          { id: 2, name: 'Basic Package', price: 2000, type: 'flat', description: 'Essential services.', features: ['4 hours coverage'] }
        ]);
        setLoading(false);
        return;
      }
  ...
  const handleShare = () => {
    const url = `https://eventore.app/vendor/${vendor.id}`;
  ...
  const portfolio = vendor?.portfolio_urls?.map(url => ({ src: url, source: 'eventore' })) || portfolioFor(vendor);
    ...
    <div className="screen-body">
      ...
      <div className="s3-content" style={{ overflowY: 'auto', paddingBottom: 8 }}>
        <div className="s3-title">
          {vendor.name}
          <span className="badge badge-premium" style={{ marginLeft: 8 }}>★ {(vendor.type || vendor.pro_status || 'PRO').toUpperCase()}</span>
        ...
        {activeTab === 'About' && (
          <div style={{ padding: '4px 4px 80px' }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--muted)', margin: 0 }}>Real-time — every confirmed Eventore booking auto-blocks this calendar.</p>
          ...
          <div style={{ padding: '4px 4px 80px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 12, marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>
                {vendor.reviews} reviews · {SAMPLE_REVIEWS.filter(r => r.verified).length} verified by Eventore bookings
              </div>
            </div>
            {SAMPLE_REVIEWS.map(r => (
              <div key={r.id} style={{ padding: '12px 0', borderTop: '1px solid var(--line)' }}>
                ...
                {r.verified && (
                  <span style={{ display: 'inline-block', marginTop: 6, fontSize: 10, fontWeight: 600,                     color: 'var(--primary)', background: 'var(--primary-soft)', padding: '2px 8px', borderRadius: 999 }}>✓ Verified by Eventore booking</span>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="s3-cta-bar">
        <div className="from">
          {selectedPkg
            ? <span>Selected <b>{selectedPkg.name}</b></span>
            : <span>From <b>${vendor.priceFrom?.toLocaleString()}</b></span>
          }
        </div>
        <Link
          to="/confirm"
          style={{ textDecoration: 'none', flex: 1, marginLeft: 12 }}
          onClick={() => updateState({ selectedPackage: selectedPkg || (packages[0] || null) })}
        >
          <button
          ...
          >
            ...
            {available ? 'Book & Inquire' : 'Unavailable on your date'}
          </button>
        </Link>
      </div>
    </div>
